...
from src.greyhounds.utils.dates import hhmm_to_today_iso
...
def _selecionar_aba_gb_ire(driver) -> None:
    try:
        ...
        logger.warning(f"Erro ao selecionar/aguardar aba GB & IRE: {exc}")


def scrape_betfair_index() -> List[Dict[str, str]]:
    logger.info("Iniciando scrape do indice da Betfair: {}", settings.BETFAIR_GREYHOUND_RACING_URL)
