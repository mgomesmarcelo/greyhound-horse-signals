"""
Utilitarios para lidar com colunas de unidades (ref = 1) e legado (_fixed_10).

Regra:
- Se o dataset ja tem colunas *_ref, usamos fator de referencia = 1.
- Caso contrario, assumimos o legado *_fixed_10 com fator = 10.
...
def get_ref_factor(df: pd.DataFrame) -> float:
    """
    Retorna 1.0 se ao menos uma coluna canonica *_ref existir (stake_ref, pnl_stake_ref, liability_ref),
    caso contrario 10.0 (legado).
    """
...
def get_col(df: pd.DataFrame, preferred: str, fallback: str):
    """
    Retorna a serie preferida se existir, senao a serie de fallback.
    Lanca KeyError se nenhuma das duas colunas existir.
    """
...
def to_bool_series(s: pd.Series) -> pd.Series:
    """
    Normaliza Series para bool: aceita bool, numeros (1/0), strings em {"true","1","t","yes","y","sim"}.
    NaN vira False.
    """
    if s is None:
