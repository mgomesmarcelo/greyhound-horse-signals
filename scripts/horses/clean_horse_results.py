...
def format_bsp_to_two_decimals(value: Any) -> str:
    if value is None:
        return ""
...
def clean_results_dir(result_dir: Path, force: bool = False) -> int:
    ...
    for csv_path in csv_paths:
        ...
        if not force and name in state:
            ...
            if fp_old.get("mtime_ns") == fp_now["mtime_ns"] and fp_old.get("size") == fp_now["size"]:
                logger.debug("Pulado (cache, sem mudancas): {}", name)
                skipped_cache += 1
                continue
        ...
        if missing:
            logger.error(
                "Pulado {}: colunas ausentes apos normalizacao ({})",
                name,
                ", ".join(missing),
            )
            continue
        ...
        if not force and is_already_clean(df):
            logger.debug("Pulado (ja limpo): {}", name)
    ...
    logger.info(
        "Resumo: alterados={}, pulados (cache)={}, pulados (ja limpos)={}",
        changed,
        skipped_cache,
...
def main(argv: list[str] | None = None) -> int:
    ...
    logger.info("Concluido. Arquivos alterados: {}", changed)
    return 0
