...
DATASETS: list[DatasetConfig] = [
    ...
    ("signals_timeform", settings.RAW_SIGNALS_TIMEFORM_DIR, settings.PROCESSED_SIGNALS_TIMEFORM_DIR, "signals_*.csv"),
    ("signals_sportinglife", settings.RAW_SIGNALS_SPORTINGLIFE_DIR, settings.PROCESSED_SIGNALS_SPORTINGLIFE_DIR, "signals_*.csv"),
    ("sportinglife_top3", settings.RAW_SPORTINGLIFE_TOP3_DIR, settings.PROCESSED_SPORTINGLIFE_TOP3_DIR, "sportinglife_top3_*.csv"),
    ("sportinglife_forecast", settings.RAW_SPORTINGLIFE_FORECAST_DIR, settings.PROCESSED_SPORTINGLIFE_FORECAST_DIR, "SportingLifeForecast_*.csv"),
]
...
def convert_file(csv_path: Path, parquet_path: Path, *, force: bool, compression: str) -> tuple[int, str]:
    if not parquet_path.exists():
        ...
        if parquet_mtime >= csv_mtime:
            logger.debug("Parquet ja existe e esta atualizado, pulando: {}", parquet_path)
            return 0, "skip"
...
def convert_dataset(dataset: DatasetConfig, *, force: bool, compression: str) -> int:
    ...
    if not raw_dir.exists():
        logger.warning("Diretorio bruto inexistente para {}: {}", name, raw_dir)
        return 0
    ...
    logger.success("Conversao {} concluida. Total de linhas: {}", name, total_rows)
    logger.debug(
        "Arquivos pulados: {} | Reconversoes por CSV mais novo: {} | Convertidos do zero: {}",
        skipped,
        reconverted,
        converted,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Converte historicos CSV dos cavalos para Parquet (raw -> processed).",
    )
    parser.add_argument(
        "--dataset",
        choices=[name for name, *_ in DATASETS],
        help="Converte apenas um dataset especifico.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Reprocessa mesmo que o Parquet ja exista.",
    )
    parser.add_argument(
        "--compression",
        default="snappy",
        help="Algoritmo de compressao Parquet (padrao: snappy).",
    )
...
def main() -> None:
    ...
    logger.info("Iniciando conversao CSV -> Parquet (horses)...")

    targets: Iterable[DatasetConfig]
    if args.dataset:
        targets = [d for d in DATASETS if d[0] == args.dataset]
    else:
        targets = DATASETS
    ...
    logger.success("Conversao finalizada. Linhas totais processadas: {}", total)
